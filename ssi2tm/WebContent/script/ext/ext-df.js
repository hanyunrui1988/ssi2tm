	...
	getRawValue : function() {
		if (!this.hoursSpinner) {
			...
			return {
				h : this.date.getHours(),
				m : this.date.getMinutes(),
				s : this.secondsSpinner ? this.date.getSeconds() : ''
			};
		} else {
			return {
				h : this.hoursSpinner.getValue(),
				m : this.minutesSpinner.getValue(),
				s : this.secondsSpinner ? this.secondsSpinner.getValue() : ''
			};
		}
	},
	...
	onRender : function(container, position) {
		...
		var tfEl = Ext.DomHelper.insertBefore(table.last(), {
			...
			children : [ {
				...
				html : this.timeLabel,
				valign : 'middle',
				style : "width:50;font-size:15px;height:20px;"
			}, {
				tag : 'td',
				cls : 'x-date-bottom ux-timefield',
				style : 'margin-left:10px;margin-bottom:5px;margin-bottom:20px;',
				colspan : '2'
			} ]
		}, true);
		...
		if (p) {
			p.setStyle("height", p.getHeight()-5);
		}
	},
	setValue : function(value) {
		var old = this.value;
...
//					this.getEl().applyStyles('bottom:0');
				});
			},
			getValue : function() {
...
//				}
				Ext.ux.form.showSeconds = this.showSeconds;
