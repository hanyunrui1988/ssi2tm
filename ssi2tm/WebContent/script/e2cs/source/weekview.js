...
// week view LOG-------------------------------------------------------
//  06/June/2010 
//  NEEDs some CSS adjustment on events rendering in day's body 
//  Changes on the Space beteween day bodies  for asthetic pourposes and view 
...
Ext.ECalendar.weekview = function(config){
	this.referid='weekview'; //0.0.9
	this.header=true; 
	this.headerButtons= true;
	this.headerFormat= 'W';
	this.headerShowDates= true; 
	this.dayformatLabel=  'D j'; 
	this.scrolltoHour= false ; 			// 0.0.15  let the ciew scroll to the current hour if any 
	this.hourFormat= 'G:i'; 				// change on 0.0.15  //  according to Ext.Date object    //0.0.7 - beta 0.0.2  changed G for H 
	this.startTime=  '00:00:00 am';		// format has to be 'H:s:i a'
	this.endTime=    '11:59:59 pm';		// format has to be 'H:s:i a'	
	this.style= 		'google';  			// style:   'plain or 'google'
	// alldatTaksMore:   
	...
	this.alldayTaksMore='window';
	this.alldayTaskTPL= new Ext.XTemplate(
	...
	this.alldayTasksMaxView=3;
	this.alldayTasks=[];
	this.weekTasks=[];
	this.store= null; 
	this.headerDayClick='none'; //'viewday' changes to day view, 'none' do nothing  // 0.0.7  beta 0.0.2 
	this.currentweek=1;  	   // dynamic change for rendering sets the actual value  
	//tasks settings for render 
	this.task_format='d-m-Y H:i:s a';
	this.task_bgcolor_base='#E0FFA2';		
	this.task_showqtip= true; 
	this.task_width=50; 
	this.tasksOffset=20;  // for overlapping tasks
	this.taskelements=0;  // private use for gen a unique id on day tasks  // added on 0.0.4 
	this.ShowMenuItems=[1,1,1,1,1,1];		//0.0.11  add, go next w , go prev w , chg month , chg day, chg sched 
	this.task_ShowMenuItems=[1,1,1,1,1];		//0.0.11  add, delete, edit, go next w, go prev w
	this.moreMenuItems=[];
	this.task_eventLaunch='click';			//0.0.11
	this.taskAdd_timer_dblclick=false; 		//0.0.15 When the user dblclick on the Time display in the body will launch an add event with the date and time selected :) 
	this.scrolltoHour= false ; 				//0.0.15 let the view scroll to the current hour if any 
	this.allday24HinsideBody=false;			//0.0.15 if the task is all day but it covers only the range from 00:00:00 am to (11:59:59 pm or  day+1 00:00:00 am) it will be rendered on the body of the day instead allday task
	this.task_clsOver='';					//0.0.11 on day view was setted up long ago but not implemented, now its implement here also 
	this.forceTaskFit= false;				//0.0.14 Mayor feature // Force all the vents to fit on the View and the width its changed gradually	
	this.startDay=1;							//0.0.14 Monday (full week number), Sunday (-1)or Saturday (-2)
	...
	//0.0.15
	this.headerTpl= new Ext.XTemplate('<tpl for=".">{title}-from {initday:this.formatx} to {enday:this.formatx}</tpl>',
	{
...
Ext.extend(Ext.ECalendar.weekview, Ext.util.Observable, {
	...
	render: function(){	
		...
		var daterange = this.getDatesforWeek(this.datetohandle); 		
		if (this.style=='plain'){ // plain style  goes on 
			...
			if (Ext.isIE){	ancho+=7; } else { 	ancho+=5;} 
		...
		for (var i=1;i<8;i++){ 
			...
			if (Ext.isOpera){ 
				...
				containerdaytmp.on('contextmenu',this.oncontextmenu_bodyday,this, 
...
//		} 
		...
		if (tableskel>test){ // scroll is on 	
			if (this.style=='plain'){  // 0.0.7  plain style fixed  for header display and pos 
				...
				ancho=0;
				for (var i=1;i<8;i++){ 	ancho+= Ext.get(this.calx.id + '-allday' + i ).getWidth(false); }
				if ( Ext.isIE ){ ancho+=7;} else { ancho+=4;} 
				myallcontainer.setWidth(ancho,false);			
				ancho=tmpgridlayout.getWidth(false);
				if ( Ext.isIE || Ext.isIE6){ 
		...
		if (this.style=='plain'){ 
		} else {//fix position of allday tasks
			var moretaskbuttonallday = Ext.get('alldaylistbutton'); 
		...
		for (var i=1;i<8;i++){ 
			...
			if (tmpcalc>tmpdayweek.getWidth(false)){ /*tmpdayweek.setWidth(tmpcalc);*/	} 
			if 	(tmpdayweek_childs.length==1 && tmpdayweek_childs[0].childNodes.length==0){ 
				...
				for (var j=0;j<childcount;j++){
					...
					if (checkel<0){ 
						...
						if (this.task_eventLaunch!=''){ tmpel.addListener(this.task_eventLaunch, this.onDblclickSingleday, this );} 
						if (this.task_clsOver!='') { tmpel.addClassOnOver(this.task_clsOver);} 
						if (Ext.isOpera){
	...
	onDblclick:function(evx,elx,obx){
		var tmpdata= Ext.get(elx.id);
		var datatask = this.getTaskarray(elx); // 0.0.14 FIX  now launches on the calendar event 
		this.calx.fireEvent("taskDblClick",datatask,this,this.calx,'week'); 				
	},
	...
	onContextTaskMenu_Hide: function (){ /*nothing happens :(*/	},	
	onContextHide_newVer: function(obj){ 
		if (Ext.version.substr(0, 1) == "3") {	 // new on 0.0.16 3x compatibility 
			obj.removeAll(); obj.destroy();
		} 
	},	
	oncontextmenu_bodyday: function(evx,elx,obx){ //0.0.16 fixed for compatibility 
		...
		if (Ext.isIE){ 
			testxxx=  Ext.get(elx.parentNode);
		...
		if (Ext.version.substr(0, 1) == "2") { evx.stopEvent(); } //0.0.16
		var tmpdata= Ext.get(elx.id);
		...
		this.menu.items.items[0].addListener('click', function(){ 
				...
				if (Ext.isIE6 || Ext.isIE7 || Ext.isIE8){ 
					if (calcdate!=undefined || calcdate  !=null){ 
						var rtndate = new Date( basedate.format('m/d/Y') + ' ' + calcdate.format('H:i:s') );
					} else { 
						var rtndate = new Date( basedate.format('m/d/Y') ); 
					} 
				} else { 
					var rtndate = new Date( basedate.format('m/d/Y') + ' ' + calcdate.format('H:i:s') ); 
				} 
		...
		if (Ext.version.substr(0, 1) == "2") { 
			...
			this.menu.on('hide', this.onContextHide_newVer, this);
	...
	oncontextmenu: function(evx,elx,obx){ // this one is for tasks 
		...
		if  (toshowOnCXmenu[2]!=true && toshowOnCXmenu[3]!=true && toshowOnCXmenu[4]!=true && newmenuitems.length<=0 ) { this.menu.items.items[2].hidden=true; } //SEPARATOR 
		if (toshowOnCXmenu[2]!=true){ 
			this.menu.items.items[3].hidden=true; //EDIT  
			this.menu.items.items[4].hidden=true; //SEPARATOR 
		} 
		if (toshowOnCXmenu[3]!=true && toshowOnCXmenu[4]!=true && newmenuitems.length<=0 ) { this.menu.items.items[4].hidden=true; } //SEPARATOR 				
		if (toshowOnCXmenu[3]!=true){	this.menu.items.items[5].hidden=true;} // NEXT WEEK
		if (toshowOnCXmenu[4]!=true){	this.menu.items.items[6].hidden=true;} //PREVWEEK 
		if (newmenuitems.length>0) { 
...
//				} 
	...
	genAlldayContainer:function(datestart){  // for google calendar style (container with task with +1 day long) 
		...
		if (taskstmp.length==0){ 
			...
			for (var j=0; j<tasktoshow; j++){ 
				...
				//changed on 0.0.4	//changed on 0.0.11
		...
		htmlcontainer+='cellpadding="0" cellspacing="0" bordercolor="#004E9B" bgcolor="#004E9B"><tr>'; //0.0.16 
		if (Ext.isIE){ 
			var anchostr = '51'; 
		} else { 
			var anchostr = '51'; 		
	...
	genBody: function(datestart){
		...
		dviewbody+='<table id="' + this.calx.id + '-week-skeleton" width="' + wtdisp + '%" border="0" cellspacing="0" cellpadding="0"' + mydatacomp + '>'; //0.0.16
