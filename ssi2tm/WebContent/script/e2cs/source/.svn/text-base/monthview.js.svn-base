Ext.ECalendar.monthview = function(config){
	this.referid='monthview';
	this.header=true;
	this.headerUseTpl=false;
	this.headerTpl=new Ext.XTemplate('<tpl for=".">{title}-{datetouse:this.formatx}</tpl>',	{	formatx:function(value){return value.format('M-Y');}});
	this.headerData={ title:'Custom header for Month', 	datetouse:new Date()};
	this.headerFormat='M-Y';
	this.headerButtons= true;
	this.moreMenuItems=[];
	this.dayAction='viewday';
	this.showTaskcount= true; 
	this.showTaskList= false;
	this.showNumTasks=5;
	this.task_format='d-m-Y H:i:s a';
	this.TaskList_launchEventOn='click';
	this.contextMenuLabels= e2cs.cal_locale.contextMenuLabelsDay;
	this.ShowMenuItems=[1,1,1,1,1,1];
	this.TaskList_ShowMenuItems=[1,1,1];
	this.TaskList_moreMenuItems=[];
	this.taskStyle='margin-top:10px;';
	this.startDay=0;
	this.thisview=null;
...
Ext.extend(Ext.ECalendar.monthview, Ext.util.Observable, {
	...
	render: function (){ 
		...
		if (this.header){
			...
			if (this.headerButtons){
				var prevclick = myheaderwrap.createChild({id:this.calx.id + '-btn-pm', tag: 'div', cls: 'x-calendar-month-previous', html:'' }); 
				var nextclick = myheaderwrap.createChild({id:this.calx.id + '-btn-nm', tag: 'div', cls: 'x-calendar-month-next',	 html:''	}); 
		...
		this.calx.height = this.calx.getInnerHeight ();
		if (!this.calx.height || this.calx.height=='undefined'){ 
			if (this.calx.getEl().dom.offsetParent!=null) { 
				var tmpheight = this.calx.getEl().dom.offsetParent.clientHeight; 
			} else { 
				var tmpheight = 0; 
			} 
			if (tmpheight ==0) { tmpheight  = 300; }
		...
		mydays.each(function(el, thisobj, index){
					...
					if ( testdx.indexOf('m-td-' + this.calx.id + '-')>=0){ 					 
							...
							if (Ext.isOpera){ 
									...
									myobjtowork.on('contextmenu',this.oncontextmenu,this, 
		...
		if (this.showTaskcount==false && this.showTaskList==true ) {
			...
			mylist_items.each(function(el,thisobj,index){
					if  ((el.id.indexOf(this.calx.id) + 1)>0){
						...
						if (this.TaskList_launchEventOn!=''){ 
						...
						if (Ext.isOpera){ 
								...
								el.on('contextmenu',this.oncontextmenuTaskitem, this,
									  {stopPropagation:true,stopEvent:true,normalized:true,preventDefault:false}
								);
						}
					} 
	...
	ZoomDay: function (DateToZoom){
		...
		tmppanel = new Ext.Panel({
			id:'ecal-more-task-panel',header:false, autoDestroy:true,autoScroll:false,monitorResize:true, border:false,autoWidth:false, autoHeight:false,
	...
	oncontextmenu: function (evx,elx,obx){ 
		...
		if (Ext.version.substr(0, 1) == "2") { evx.stopEvent(); }		
		if (elx.className=='noday' || elx.className=='today' || elx.className=='monthday') { 
			...
			if(elx && elx.firstChild && elx.firstChild.id){
			var refdate = new Date( elx.firstChild.id.substring(this.calx.id.length + 1) ); 
		}			
		}			
		if (!this.menu){ 
			this.menu = new Ext.menu.Menu({
				id: this.calx.id + '-contextmenu-month',
				autoDestroy:false, shadow: true, 
				items:[
				{
					id: this.calx.id + '-month_ctxbtn_task',
					iconCls:'x-calendar-month-btnmv_task',
					autoDestroy:false,
					text: e2cs.cal_locale.contextMenuLabelsMonth.task,
					scope:this,
					functionID : 'RCZB_PBXX_EDIT'
				}
				]
			});
		}
		if (this.moreMenuItems.length>0) { 
			...
			for (var i=0; i< this.moreMenuItems.length; i++){
				...
				if (this.moreMenuItems[i].hasListener('click')){
					this.moreMenuItems[i].purgeListeners(); 
				}
		...
		if(this.menu.items.items[0].hasListener('click')){
			this.menu.items.items[0].purgeListeners();
		}
		...
		if (Ext.version.substr(0, 1) == "2") {	
			...
			this.menu.on('show', this.onContextShowBodyDAY, this);
	...
	onCustomMenuAction: function(MenuId,MonthEl,TaskObj){
		...
		if (MonthEl.dom.className=='noday' || MonthEl.dom.className=='today' || MonthEl.dom.className=='monthday') { 
			datatask[0] = Ext.get(MonthEl).dom.id.substring(this.calx.id.length + 1);
			datatask[1] = Ext.get(MonthEl).getAttributeNS('tag','class');
		} else if (MonthEl.dom.className=='tasks' ){ 
			datatask[0] = Ext.get(MonthEl).dom.parentNode.firstChild.id.substring(this.calx.id.length + 1);
			datatask[1] = Ext.get( MonthEl.dom.parentNode.firstChild ).getAttributeNS('tag','class');
		} else { 
			datatask[0] = Ext.get(MonthEl).dom.firstChild.id.substring(this.calx.id.length + 1);
			datatask[1] = Ext.get( MonthEl.dom.firstChild ).getAttributeNS('tag','class'); 
		}  
	...
	onTaskAdd: function(objx, mviewx){
		var tmpClass = objx.dom.className.trim();
		if (tmpClass=='noday' || tmpClass=='today' || tmpClass=='monthday') { 
			var refdate= new Date(objx.id.substring(this.calx.id.length + 1) );
		} else if (tmpClass=='lunar_monthday' || tmpClass=='lunar_jieqi_monthday' || tmpClass=='lunartoday' || tmpClass == 'lunar_jieqi_today') { 
			var refdate= new Date(objx.id.substring(this.calx.id.length + 1) );
		} else if (tmpClass=='tasks' ){ 
			var refdate = new Date(objx.dom.parentNode.firstChild.id.substring(this.calx.id.length + 1) );
		} else if (tmpClass=="tasks_list_item" || tmpClass=="tasks_list_item task_list_item_over" || tmpClass.indexOf("task_list_item_over")>0 ){
			tmpdata =  objx.id.replace(this.calx.id + "-tasklist-",'');
			refdate =  new Date(tmpdata + "" ); 
		} else if (tmpClass=="tasks_list"){
			tmpdata =  objx.id.replace(this.calx.id + "-tasklist-",'');
			var refdate =  new Date(tmpdata + "" ); 
		} else if (tmpClass == 'daybody'){
			tmpdata =  objx.id.replace("m-td-" + this.calx.id + "-",'');
			refdate =  new Date(tmpdata + "" ); 
		} else { 
			if(objx.dom && objx.dom.firstChid && objx.dom.firstChild.id){
	...
	onContextShowBodyDAY: function(obj){ 
		if(!this.menuEnabled){
			for(var i = 0; i<obj.items.length;i++){
				obj.items.items[i].disable();
			}
		}
	},	
	onContextHide: function(){},
	onContextHide_Taskitem: function(menuItem){},	
	operataskitembuttons: function (evx,elx,obx){
		if (Ext.isOpera){ 
				evx.stopEvent();
				...
				if (testevent==false) {
					if 	(actionsTaskCX[0]==false){ 
						if 	(actionsTaskCX[1]==true){ 
							if (actionsTaskCX[2]==true){ 
								var newmenuitems = actionsTaskCX[3];
							} 
							var toshowOnCXmenu = actionsTaskCX[4]; 
						} else {
							var newmenuitems   = this.TaskList_moreMenuItems; 					
							var toshowOnCXmenu = this.TaskList_ShowMenuItems; 
						} 
				...
				if (newmenuitems.length>0) { 
					...
					for (var i=0; i<newmenuitems.length; i++){
						...
						if (newmenuitems[i].menu==undefined) { 
							if (newmenuitems[i].ctype=="Ext.menu.Item"){ 
								newmenuitems[i].rendered=false;
								if  (newmenuitems[i].hasListener('click')){
							...
							for (var xm=0;xm<newmenuitems[i].menu.items.length;xm++){
								...
								if (newmenuitems[i].menu.items.items[xm].ctype=="Ext.menu.Item"){ 
									if  (newmenuitems[i].menu.items.items[xm].hasListener('click')){
				...
				if(this.taskitem_menu.items.items[0].hasListener('click')){
					this.taskitem_menu.items.items[0].purgeListeners();
				}
				this.taskitem_menu.items.items[0].addListener('click', function(){ this.onActionTask( 1, Ext.get(elx), this ); 	}, this); 
				if(this.taskitem_menu.items.items[1].hasListener('click')){
					this.taskitem_menu.items.items[1].purgeListeners();
				}
				this.taskitem_menu.items.items[1].addListener('click', function(){ this.onActionTask( 2, Ext.get(elx), this ); 	}, this); 
				if(this.taskitem_menu.items.items[3].hasListener('click')){
					this.taskitem_menu.items.items[3].purgeListeners();
				}
				...
				if (Ext.version.substr(0, 1) == "2") {	
					...
					evx.xy = [evx.clientX, evx.clientY];
				} 
				this.taskitem_menu.showAt(evx.xy);			
		}
	},
	onDblClick_tasklistitem: function(evx,elx,obx){ 
		var test=elx.id.indexOf( this.calx.id + '-mtask-item-')+1;
		if (test){ 
			var datatask = this.getTaskarray(elx); 
			this.calx.fireEvent("taskDblClick",datatask,this,this.calx,'month'); 
		} 
	},
	oncontextmenuTaskitem:function(evx,elx,obx){
		if (Ext.isOpera){ if (evx.button!=2){ return false; } }
		if (Ext.version.substr(0, 1) == "2") { evx.stopEvent(); }
		if (this.TaskList_ShowMenuItems[0]!=true && this.TaskList_ShowMenuItems[1]!=true && this.TaskList_ShowMenuItems[2]!=true && this.TaskList_moreMenuItems.length<=0 ){ return false;} 
		var tmpdata= Ext.get(elx.id); 		
		var toshowOnCXmenu = this.TaskList_ShowMenuItems;
		var newmenuitems   = this.TaskList_moreMenuItems; 
		var actionsTaskCX=[]; 
		...
		if (testevent==false) {
			if 	(actionsTaskCX[0]==false){ 
				if 	(actionsTaskCX[1]==true){ 
					if (actionsTaskCX[2]==true){ var newmenuitems = actionsTaskCX[3]; }
					var toshowOnCXmenu = actionsTaskCX[4]; 
				} else {
					var newmenuitems   = this.TaskList_moreMenuItems; 					
					var toshowOnCXmenu = this.TaskList_ShowMenuItems; 
		...
		if (!this.taskitem_menu){
			this.taskitem_menu = new Ext.menu.Menu({
				...
				shadow: true, 
				items:[{id:'month_tskitem_btn_task-add',	iconCls:'x-calendar-day-btnmv_add',		text: this.contextMenuLabels.taskAdd, 	scope:this},
					   {id:'month_tskitem_btn_task-edit',	iconCls: 'x-calendar-day-btnmv_task',	text: this.contextMenuLabels.taskEdit,scope:this},
					   '-',
					   {id:'month_tskitem_btn_task-delete', iconCls: 'x-calendar-day-btnmv_delete',	text: this.contextMenuLabels.taskDelete,scope:this}
				]
			});
		...
		if (newmenuitems.length>0) { 
			...
			for (var i=0; i<newmenuitems.length; i++){
				...
				if (newmenuitems[i].menu==undefined) { 
					...
					if (newmenuitems[i].menu){ newmenuitems[i].menu.rendered=false; } 					
		...
		if (toshowOnCXmenu[0]!=true){  this.taskitem_menu.items.items[0].hidden=true;} 
		if (toshowOnCXmenu[1]!=true){  this.taskitem_menu.items.items[1].hidden=true;} 
		if (toshowOnCXmenu[0]!=true && toshowOnCXmenu[1]!=true){ this.taskitem_menu.items.items[2].hidden=true;} 
		...
		if (Ext.version.substr(0, 1) == "2") { 
			this.taskitem_menu.on('hide', this.onContextHide_Taskitem, this);
			this.taskitem_menu.showAt( [ evx.getPageX(), evx.getPageY() ] );
		} else { 
			this.taskitem_menu.on('show', this.onContextShowBodyDAY, this);
			this.taskitem_menu.showAt([evx.clientX, evx.clientY]);
		}
	}, 
	...
	genHeader: function(dateval){
		...
		Date.monthNames = e2cs.cal_locale.monthtitles; 
		var myheader = '<div class="x-calendar-month-header" style="width:' + (this.calx.width) + 'px;">';
	...
	getDateRangeOfMonth: function(dateval){ 
		var dt= new Date(dateval); 													
		var initday = dt.getFirstDayOfMonth()-this.startDay;
		...
		var icount = initday; 
		startmonthdate = new Date( dt.format('m/01/Y') );
		var icount = (initday + daysgen) % 7;
		if (icount) icount = 7 - icount;
		var datenew = new Date(startmonthdate).add(Date.DAY, daysgen - 1 + icount);
	...
	genBody: function(dateval,configdata){ 
			...
			for(var iday=(initday); iday>0; iday--){
				...
				if (this.showTaskcount){
					...
					if (this.showTaskList) { 
							...
							if (taskitems!=''){	
								mybody+='<div class="tasks_list" id="' +  this.calx.id + '-tasklist-' + writedt.format('m/d/Y') + '" style="height:10px;' + configdata.styletasks + '">';
								mybody+=taskitems; 
								mybody+='</div>'; 	
							} 
					} 
		...
		for (var imonth=0; imonth<daysgen; imonth++){ 
			...
			var lunarTodayStyle = 'lunartoday';
			var lunarMonthdayStyle = 'lunar_monthday';
			var lunar = new x(datecreate);
			if(lunar.jieqi != ''){
				lunar = lunar.jieqi;
				lunarTodayStyle = 'lunar_jieqi_today';
				lunarMonthdayStyle = 'lunar_jieqi_monthday';
			} else lunar = lunar.lunarDateInChinese;
			if (datecreate.format('m/d/Y')==dt.format('m/d/Y') ){ 
				mybody+='<td valign="top" id="m-td-' + this.calx.id + '-' + datecreate.format('m/d/Y') + '" class="daybody" height="5px">'; 
				mybody+='<span class="today" id="'  + this.calx.id + '-' + datecreate.format('m/d/Y') + '">' +  datecreate.format('j') + '日'+  '</span>'; 
				mybody+='<span class="'+lunarTodayStyle+'" id="'  + this.calx.id + '-' + datecreate.format('m/d/Y') + '">' +  lunar +   '</span>'; 
				if (this.showTaskcount){
					...
					if (this.showTaskList) { 
							...
							if (taskitems!=''){	
								mybody+='<div class="tasks_list" id="' +  this.calx.id + '-tasklist-' + datecreate.format('m/d/Y') + '" style="height:10px;' + configdata.styletasks + '">';
								mybody+=taskitems; 
								mybody+='</div>'; 	
							} 
					} 
				...
				mybody+='<td valign="top" id="m-td-' + this.calx.id + '-' + datecreate.format('m/d/Y') + '" class="daybody" height="5px">';
				mybody+='<span class="monthday" id="' + this.calx.id + '-' + datecreate.format('m/d/Y') + '">' +  datecreate.format('j') +'日'+   '</span>'; 
				mybody+='<span class="'+lunarMonthdayStyle+'" id="' + this.calx.id + '-' + datecreate.format('m/d/Y') + '">' +  lunar +   '</span>';
				if (this.showTaskcount){
						tmptasks = this.getCountTask(datecreate, this.calx.store); 
						if (tmptasks>0 ){ 
							mybody+='<div class="tasks" style="' + configdata.styletasks + '">' + e2cs.cal_locale.labelforTasksinMonth + ' (' + tmptasks + ')</div>'; 
		...
		if (icount<7 ){ 
			...
			for (var iday=icount; iday<7; iday++){
				...
				mybody+='<td valign="top"   id="m-td-' + this.calx.id + '-' + datenew.format('m/d/Y') + '" class="daybody" height="5px">';
	...
	getCountTask:function(dtx,storex){
		...
		if ( counttasks>0 ){ 
			var count_in_day=0; 
			for (var itask=0; itask<counttasks ; itask++){
				dateinit = storex.getAt(itask).data[this.calx.fieldsRefer.startdate];  
				dateend =  storex.getAt(itask).data[this.calx.fieldsRefer.enddate];
				checkdates = dtx.between( new Date(dateinit), new Date(dateend) ); 
				chkformat =  dtx.format('m/d/Y'); 
				test = new Date(dateinit); if (test.format('m/d/Y')==chkformat){  checkdates =true; } 
				test = new Date(dateend);  if (test.format('m/d/Y')==chkformat){  checkdates =true; } 
	...
	getTaskList:function(dtx,storex){ 
		var fields = this.calx.fieldsRefer;
		var daystoreturn = ""; 
		var counttasks= storex.getCount();
		if ( counttasks>0 ){ 
			var count_in_day=0; 
			for (var itask=0; itask<counttasks ; itask++){
				var data = storex.getAt(itask).data;
				dateinit = data[fields.startdate];
				dateend =  data[fields.enddate];
				checkdates = dtx.between( new Date(dateinit), new Date(dateend) ); 
				chkformat =  dtx.format('m/d/Y'); 
				test = new Date(dateinit); if (test.format('m/d/Y')==chkformat){  checkdates =true; } 
				test = new Date(dateend);  if (test.format('m/d/Y')==chkformat){  checkdates =true; } 
				...
				if (checkdates){ 
					if (count_in_day<this.showNumTasks){
						var tmpdate = new Date(data[fields.startdate]);			
						var startlabel = tmpdate.format(this.task_format); 	
						tmpdateb = new Date(data[fields.enddate]);
						var endlabel = tmpdateb.format(this.task_format); 
						var tmpdescription = data[fields.description], tmpdescriptionx;
						if (tmpdescription == '')
							tmpdescription = '&#160;';
						if (tmpdescription.length>this.calx.tipmaxLength){ 
							tmpdescriptionx = tmpdescription.substring(0,this.calx.tipmaxLength) + '...'; 
						} else {
							tmpdescriptionx = tmpdescription; 
						}
						daystoreturn+='<div class="tasks_list_item"';
						daystoreturn+='style="background-color:' + data[fields.color];
						if (fields.textcolor) 
							daystoreturn += ';color:' + data[fields.textcolor]; 
						daystoreturn += '" ';
						daystoreturn+='id="' + this.calx.id + '-mtask-item-' + chkformat + '-' + data[fields.id] + '" ';
						daystoreturn+='ec_storeindex="' + itask + '" ';
						daystoreturn+='ec_id="' + data[fields.id] + '" ';
						daystoreturn+='ec_starts="' + startlabel + '" ';
						daystoreturn+='ec_ends="' + endlabel + '" ';
						daystoreturn+='ec_zbr="' + data[fields.zbr] + '" ';
						daystoreturn+='ec_subject="' + data[fields.subject] + '" ';
						daystoreturn+='ec_cnt="' + tmpdescriptionx + '" ';
						daystoreturn+='ec_bgdh="' + data[fields.bgdh] + '" ';
						daystoreturn+='ec_yddh="' + data[fields.yddh] + '" ';
						daystoreturn+='ec_flid="' + data[fields.flid] + '" ';
						daystoreturn+='ec_flmc="' + data[fields.flmc] + '" ';
						daystoreturn+='ec_xh="' + data[fields.xh] + '" ';
						daystoreturn+='ec_ryid="' + data[fields.ryid] + '" ';
						daystoreturn+='ec_zbzq="' + data[fields.zbzq] + '" ';
						daystoreturn+='ec_display="' + data[fields.display] + '" ';
						var datatip = {
						   ...
						   textcolor:	 (fields.textcolor ? data[fields.textcolor] : ''),
						   zbr: data[fields.zbr],
						   subject : data[fields.subject],
						   eventid: data[fields.id],
						   bgdh : data[fields.bgdh] == ''?'暂无':data[fields.bgdh],
						   yddh : data[fields.yddh] == ''?'暂无':data[fields.yddh],
						   flid : data[fields.flid],
						   flmc : data[fields.flmc],
						   xh : data[fields.xh],
						   ryid : data[fields.ryid],
						   zbzq : data[fields.zbzq],
						   display : data[fields.display]
						};
						var mylist_itemTip =  this.calx.tplTaskTip.apply(datatip); 						
						daystoreturn+='ext:qtip="' + mylist_itemTip + '">';
						daystoreturn+= data[fields.display] + '</div>';
						count_in_day+=1;
					}
				} 
			} 
	...
	getTaskarray: function(TaskElx){ 
		...
		datatask[7]=tmpdata.getAttributeNS('tag','ec_cnt') ;
		datatask[8]=tmpdata.getAttributeNS('tag','ec_bgdh') ;
		datatask[9]=tmpdata.getAttributeNS('tag','ec_yddh') ;
		datatask[10]=tmpdata.getAttributeNS('tag','ec_flid') ; 
		datatask[11]=tmpdata.getAttributeNS('tag','ec_flmc') ; 
		datatask[12]=tmpdata.getAttributeNS('tag','ec_xh') ;
		datatask[13]=tmpdata.getAttributeNS('tag','ec_ryid') ;
		datatask[14]=tmpdata.getAttributeNS('tag','ec_zbzq') ;
		datatask[15]=tmpdata.getAttributeNS('tag','ec_display') ;
		return 	datatask; 	
	},
	setMenuEnabled : function(enabled){
		this.menuEnabled = enabled;
	},
	getMenuEnabled : function(){
