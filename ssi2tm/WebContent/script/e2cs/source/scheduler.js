...
Ext.ECalendar.scheduleview = function(config){
	this.referid='schedule';
	this.header=true;
	this.headerFormat='M-Y';
	this.headerButtons=true;
	this.headerAction='gotoview';
	this.periodSelector=false;
	this.blankzonebg='#6C90B4';
	this.blankHTML='';
	this.listItems={ 
	...
	this.contextMenuLabels=e2cs.cal_locale.contextMenuLabelsSchedule;
	this.thisview=null;
					...
					if (el.id.indexOf(this.calx.id + '-eventno-')+1>=1){					 
							...
							if (this.listItems.editableEvents){ // Menu Config Listener the List items Here 
								if (Ext.isOpera){ 
									...
									el.on('contextmenu',this.oncontextmenu_for_lisItem, this,
	...
	oncontextmenu_for_lisItem: function(evx,elx,obx){
		...
		if (this.listItems.moreMenuItems.length>0) { 
			for (var i=0; i< this.listItems.moreMenuItems.length; i++){
				...
				if  (this.listItems.moreMenuItems[i].hasListener('click')){  // remove previous listeners added 0.0.16 bug fix
					this.listItems.moreMenuItems[i].purgeListeners(); 
				}	
		...
		if (Ext.version.substr(0, 1) == "2") { 
			...
			this.menu_listItem.on('hide', this.onContextHide_newVer, this);
			this.menu_listItem.showAt([evx.clientX, evx.clientY]);
		}
//		this.menu_listItem.on('hide', this.onContextHide, this);
//		this.menu_listItem.showAt( [ evx.getPageX(), evx.getPageY() ]);
	},
	oncontextmenu_for_taskdd:function(evx,elx,obx){ //0.0.16 3x fixed  
		...
		if (Ext.version.substr(0, 1) == "2") { 
			...
			this.taskmenu_listItem.on('hide', this.onContextHide_newVer, this);
			this.taskmenu_listItem.showAt([evx.clientX, evx.clientY]);
		}
		//this.taskmenu_listItem.on('hide', this.onContextHide, this);
		//this.taskmenu_listItem.showAt( [ evx.getPageX(), evx.getPageY() ]);		
	},
		...
		for (var i= 0; i<cuenta; i++){  
					...
					if (this.listItems.editableEvents){ // Menu Config Listener the List items Here 
						if (Ext.isOpera){ 
							...
							mynewElement.on('contextmenu',this.oncontextmenu_for_taskdd, this,
								  {stopPropagation:true,stopEvent:true,normalized:true,preventDefault:true}
							);	 //new change though it seems  equal than previous version
